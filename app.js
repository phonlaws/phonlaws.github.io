...
const modeUser = $("#modeUser");
const modeAdmin = $("#modeAdmin");
...
function overdueMinutes(){
  if(!overdueMinutesInput || isKiosk){
    return Number(state.overdueMinutes || 120);
  }
...
function setAdminMode(on){
  if(adminUserWrap){
    adminUserWrap.style.display = on ? "block" : "none";
  }
...
function showLogin(msg){
  ...
  if(loginHint){
    loginHint.textContent =
      msg || "กรุณาเลือกชื่อผู้ใช้และกรอก PIN 6 หลัก";
  }
...
function setLoginState(user, role){
  ...
  if(loginChip){
    loginChip.style.display = currentUser ? "inline-flex" : "none";
  }
...
function bindLoginUI(){
  ...
  modeUser?.addEventListener("change", () => {
    setAdminMode(false);
  });

  modeAdmin?.addEventListener("change", () => {
    setAdminMode(true);
  });
  ...
  loginBtn?.addEventListener("click", async () => {
    ...
    try{
      ...
      if(loginErr){
        loginErr.textContent = msg.includes("pin")
          ? "PIN ไม่ถูกต้อง"
...
function renderJobs(){
  ...
  state.jobs.forEach(job => {
    ...
    const canClose =
      (!isKiosk) &&
      (currentUser && (isAdmin() || owner === currentUser));
      ...
      <div class="card-top">
        <div class="badges">
          <span class="badge ${badgeClass}">
            <span class="dot"></span> ${getRiskLabel(job.riskType)}
          </span>
          <span class="badge dept">
            หน่วยงาน: <b style="color:var(--text)">${escapeHtml(job.department)}</b>
          </span>
          ${overdueBadge}
        </div>
        ${closeBtn}
...
function renderSummary(){
  ...
  DEPARTMENTS.forEach(d => {
    counts[d] = { confined:0, height:0 };
  });

  state.jobs.forEach(j => {
    if(!counts[j.department]){
      counts[j.department] = { confined:0, height:0 };
    }
...
function tickElapsed(){
  document.querySelectorAll(".elapsed").forEach(el => {
    ...
    if(job){
      el.textContent = formatDuration(secondsSince(job.startedAtISO));
    }
  });
...
async function apiGetStatus(){
  ...
  if(hasLoadedOnce){
    for(const id of nextIds){
      if(!prevIds.has(id)){
        flashUntil.set(id, Date.now() + FLASH_MS);
      }
    }
  }else{
    hasLoadedOnce = true;
...
function bindForm(){
  ...
  clearBtn?.addEventListener("click", () => {
    ...
    const controlInput = document.getElementById("controlMeasure");
    if(controlInput){
      controlInput.value = "";
    }
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const riskType =
      document.querySelector('input[name="riskType"]:checked')?.value ||
      "confined";

    const department = $("#department")?.value;
    const point = $("#workPoint")?.value.trim();

    // ✅ แก้ตรงนี้: ใช้ controlMeasure แทน control
    ...
    if(!department){
      showToast('กรุณาเลือกหน่วยงาน');
      return;
    }

    if(!point){
      showToast('กรุณากรอกจุดงาน');
      return;
    }
    ...
    try{
      ...
      // ✅ แก้ตรงนี้: เคลียร์ controlMeasure แทน control
      const controlInput = $("#controlMeasure");
      if(controlInput){
        controlInput.value = '';
      }

      // ✅ เคลียร์ checkbox multi select หลังเปิดงาน
      document.querySelectorAll(".ctrl-chip input").forEach(c => {
        c.checked = false;
      });
  ...
  overdueMinutesInput?.addEventListener('change', async () => {
    ...
    let v = parseInt(String(overdueMinutesInput.value || "").trim(), 10);
    ...
    overdueMinutesInput.value = String(v);

    try{
      await apiPost('/api/config', { overdueMinutes: v });
      await apiGetStatus();
      showToast("ตั้งค่าเวลาเกินเวลาแล้ว");
    }catch(err){
      showToast(String(err.message || err));
    }
  });
...
function applyRiskTheme(){
  ...
  const set = () => {
    ...
    document.body.dataset.risk = type;
  };
